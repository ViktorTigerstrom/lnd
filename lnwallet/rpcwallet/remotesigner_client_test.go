...
// GetStreamShouldFailure returns the current value of the streamShouldFail
// boolean.
...
// client and the watch-only node. This should be called right after the stream
// has been created, and before any listener has picked up stream creation
// signal over the streamCreated channel.
...
// TestRemoteSignerClient_PingResponse that we can send a ping request to the
// remote signer client, and that it will respond with a pong.
...
// TestRemoteSignerClient_MultiplePingResponses that we can send multiple ping
// requests to the remote signer client, and that it will respond with a pong
// for each request.
...
// TestRemoteSignerClient_StreamRecvErrorHandling tests that the remote signer
// client will cancel the stream if an error is received over the stream.Recv()
// method.
...
// TestRemoteSignerClient_StreamRecvErrorHandling tests that the remote signer
// client will return a SignerError if it cannot process a received request.
...
// TestRemoteSignerClient_StreamCreationBackoffTest Tests that the client will
// retry to create a stream if the stream creation fails, and that the backoff
// duration before retrying to setup the stream again is increased with each
// retry.
