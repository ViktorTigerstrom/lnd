...
//
// TODO: Replace the getAccounts func with opts of acounts.
func (s *SignCoordinator) Run(stream StreamServer,
	...
	err := s.handshake(stream, getAccounts)
	if err != nil {
		return err
	}
...
func (s *SignCoordinator) handshake(stream StreamServer,
	getAccounts func() ([]*walletrpc.Account, error)) error {
	...
	complete := &walletrpc.RegistrationResponse_RegistrationComplete{
		...
		RegistrationComplete: &walletrpc.RegistrationComplete{
			...
			WalletInfo: &walletrpc.RegistrationComplete_Accounts{
				Accounts: &walletrpc.ListAccountsResponse{
					Accounts: accounts,
				},
			},
		},
...
func (s *SignCoordinator) ForwardLocalCommitment(ctx context.Context,
...
//
// Note: this is part of the RemoteSignerInformer interface.
func (s *SignCoordinator) ForwardMuSig2Info(ctx context.Context,
	muSig2SessionId []byte, commitTx *wire.MsgTx,
...
//
// Note: this is part of the RemoteSignerInformer interface.
func (s *SignCoordinator) ForwardFundingInfo(ctx context.Context,
	fundingPoint *wire.OutPoint, localChanCfg *channeldb.ChannelConfig,
