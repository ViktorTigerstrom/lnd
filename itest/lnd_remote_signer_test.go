...
func getRemoteSignerTestCases() []remoteSignerTestCase {
...
func testInboundRemoteSigner(ht *lntest.HarnessTest) {
	...
	for _, testCase := range getRemoteSignerTestCases() {
		subTest := testCase
		...
		*node.HarnessNode, *node.HarnessNode) {
		...
		watchOnly := st.CreateNewNode(
			"WatchOnly", append([]string{
				...
				"--remotesigner.inboundconnectiontimeout=30s",
			}, commitArgs...),
			password,
		)
		...
		signerArgs := []string{
			...
			"--remotesigner.inboundconnectiontimeout=10s",
			fmt.Sprintf(
				"--remotesigner.rpchost=localhost:%d",
