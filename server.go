...
func (s *server) createLivenessMonitor(cfg *Config, cc *chainreg.ChainControl) {
	...
	if s.cfg.RemoteSigner != nil && s.cfg.RemoteSigner.Enable {
		subCfgValue := extractReflectValue(
			cfg.SubRPCServers.WalletKitRPC,
		)

		rsv := subCfgValue.FieldByName("RemoteSigner")
		if !rsv.IsNil() && rsv.IsValid() {
			rs, ok := rsv.Interface().(rpcwallet.RemoteSigner)

			if ok {
				timeout := cfg.HealthChecks.RemoteSigner.Timeout
				// Because we have two cascading timeouts here,
				// we need to add some slack to the "outer" one
				// of them in case the "inner" returns exactly
				// on time.
				...
				rsConnectionCheck := healthcheck.NewObservation(
					...
					rpcwallet.HealthCheck(
						rs,
						// For the health check we might
						// to be even stricter than the
						// initial/normal connect, so we
						// use the health check timeout.
						timeout,
					),
